...
export default function usePeer() {
  ...
  const { sendMessage, lastMessage, readyState } = useWebSocket('http://146.190.192.108:5000/', {
    heartbeat: HEARTBEAT,
    onError: (_e) => {
        ...
        if (event === MESSAGE_EVENTS.MATCH) {
          ...
          try {
            ...
            dataConnection.on('close', () => {
              ...
              clearMessages();
              sendMessage(JSON.stringify({ event: MESSAGE_EVENTS.SKIP, id: myPeerId }));
            });
  ...
  const send = (e: { preventDefault: () => void; target: { value: string }[] }) => {
    e.preventDefault();
